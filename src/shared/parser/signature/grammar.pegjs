...
ConstructorContext
  = (&Lu Identifier _ ".")* &Lu Identifier ":" __ "(" dom:(_ TypeLabeled _ ","?)* ")" __ "=>" __ cod:TypeLabeled _
{
  const results = [];
  for (const t of dom) results.push(t[1]);
  results.push(cod);
  return results;
}
  / (&Lu Identifier _ ".")* &Lu Identifier ":" __ !"(" atom:TypeLabeled atoms:(__ "=>" __ TypeLabeled)* _
{
  const results = [];
  results.push(atom);
  for (const t of atoms) results.push(t[3]);
  return results;
}

FunctionContext
  = ((&Lu Identifier _ ".")* &Ll Identifier / Parentheses) ":" __ atom:TypeLabeled atoms:(__ "=>" __ TypeLabeled)* _
{
  const results = [];
  results.push(atom);
  for (const t of atoms) results.push(t[3]);
...
TypeApplication
  = head:TypeIdentifier spine:(_ !"=>" TypeAtom)*
{
  let buffer = "";
  buffer += head;
  for (const atom of spine) {
    buffer += atom[0]
    buffer += atom[2]
  };
  return buffer;
...
TypeAtomSansParentheses
  = TypeIdentifier
  / TypeCompositeSansParentheses
...
TypeLabeled
  = label:(&Ll Identifier "::") atom:TypeNonLabeled qualifier:"?"?
{
  let buffer = "";
  buffer += label[1];
  buffer += label[2];
  buffer += atom;
  buffer += qualifier ? qualifier : "";
  return buffer;
}
  / TypeNonLabeled
...
TypeQualified
  = path:(&Lu Identifier _ ".")* &Ll leaf:Identifier
{
  let buffer = "";
  for (const p of path) {
    buffer += p[1];
...
TypeWithSpace
  = atom:TypeLabeled atoms:(__ "=>" __ TypeLabeled)*
{
  let buffer = "";
  buffer += atom;
  ...
  = start:IdentifierStart parts:IdentifierPart*
{
  let buffer = "";
  buffer += start;
  for (const part of parts) buffer += part;
  return buffer;
}
