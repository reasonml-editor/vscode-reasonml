...
function handler(languageClient: client.LanguageClient): ({ position: positionWire, uri }: types.ILocatedPosition) => Promise<string> {
  return async ({ position: positionWire, uri }) => {
    ...
    const positionCode = languageClient.protocol2CodeConverter.asPosition(positionWire);
...
export function register(context: vscode.ExtensionContext, languageClient: client.LanguageClient): void {
  ...
  languageClient.onRequest(remote.client.giveWordAtPosition.method, handler(languageClient));
}
